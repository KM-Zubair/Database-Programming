...
-- 1.	Displaying the name and salary of all doctors who work in Neurology and Rehab area. 

SELECT AREA, DOC_NAME, SALPERMON
FROM DOCTOR
WHERE AREA = 'Neurology' OR AREA = 'Rehab' 
ORDER BY AREA, SALPERMON;



-- 2. 	Displaying the ID, name and username for all doctors. 

SELECT DOC_ID, DOC_NAME, CONCAT(SUBSTR(DOC_NAME, 1, 3) , SUBSTR(DOC_ID,1,3)) AS USERNAME 
FROM DOCTOR
;



-- 3.   A query that will calculate the number of years that the doctors have been working

select DOC_ID, DOC_NAME,((SELECT floor(months_between(current_date, doctor.datehired) /12) from dual)) AS No_of_Years
from DOCTOR;



-- 4.   A query that will calculate the number of months that each doctor has been hired in the database

select DOC_ID, DOC_NAME,((SELECT floor(months_between(current_date, doctor.datehired)) from dual)) AS "Months in Service"
from DOCTOR;
